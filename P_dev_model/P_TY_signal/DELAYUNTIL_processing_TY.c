...
void processing_TY_signal_DP_TY(u8 num_ty, S_state_TY* p_status_TY) {
	...
	while(1){
		...
		if(counter>=ps_TY_user_config->puls_with)// если пройшло время импульса, выход
		{
			break;
		}
		counter+=TIME_TOTAL_CHECK_V;
		// запускаю ф-ю проверки наличия напряжения оперативного тока
		//if(processing_TY_signal_chek_processing(&processing_TY_signal_check_v,ERROR_V_INPUT,p_status_TY,num_ty))
		//{
